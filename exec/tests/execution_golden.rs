...
fn execution_snapshot_hash_golden() -> Result<()> {
    let expected = read_expected(concat!(env!("CARGO_MANIFEST_DIR"), "/tests/execution_golden_hashes.txt"))?;

    let f = File::open(concat!(env!("CARGO_MANIFEST_DIR"), "/tests/data/core_exec_golden.jsonl"))?;
