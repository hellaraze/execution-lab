...
fn main() -> anyhow::Result<()> {
    ...
    if guard.allow_event() {
        let ev = ExecEvent::OrderCreated { id: OrderId(1) };
    ...
    // В реальном мире здесь приходит snapshot с биржи/источника.
    // Для нашего контура: snapshot = барьер, после которого можно снова принимать события.
    ...
    if guard.allow_event() {
        let ev = ExecEvent::OrderAcked { id: OrderId(1) };
        w.append_bytes("event", now_ns(), &serde_json::to_vec(&ev)?)?;
        live_events.push(ev);
    }
