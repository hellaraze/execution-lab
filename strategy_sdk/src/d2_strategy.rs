...
use d2::{compute_signal, Fees, Thresholds, GasDecision, DecisionReason, SpreadInput};

pub struct D2SignalStrategy;

impl Strategy for D2SignalStrategy {
    fn name(&self) -> &'static str { "d2_signal" }

    fn compute(&self, _ctx: &StrategyContext, input: &StrategyInput) -> (GasDecision, DecisionReason) {
        let si = SpreadInput {
