...
fn exec_events_hash_golden() {
    ...
    let out = std::process::Command::new("cargo")
        .args(["run", "-q", "-p", "replay", "--bin", "exec_hash", "--", log_path])
        .output()
        .expect("run exec_hash");
    assert!(out.status.success(), "exec_hash failed: {}", String::from_utf8_lossy(&out.stderr));
    ...
    let golden = fs::read_to_string(concat!(env!("CARGO_MANIFEST_DIR"), "/tests/data/exec_events_hash.txt"))
