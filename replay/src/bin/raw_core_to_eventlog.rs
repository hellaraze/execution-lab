...
fn main() -> Result<()> {
    ...
    let out_path = std::env::args()
        .nth(2)
        .unwrap_or_else(|| "events.eventlog".to_string());

    let f = File::open(&in_path).with_context(|| format!("open raw {}", in_path))?;
    let r = BufReader::new(f);

    let mut w = EventLogWriter::open_append(&out_path, "raw_core", Durability::Buffered)
    ...
    for line in r.lines() {
        ...
        let mut v: serde_json::Value = serde_json::from_str(&s)
            .with_context(|| format!("parse raw core json line {}", n + 1))?;
    ...
    println!("raw_core_to_eventlog ok: in={} out={} n={}", in_path, out_path, n);
    Ok(())
}
